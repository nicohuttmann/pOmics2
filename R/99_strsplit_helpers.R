...
#'
#'
.strsplit <- function(x, split, vector.output = T, as_numeric = T) {
  ...
  if (is.list(x) || is.atomic(x)) {
    ...
    stop("Unknown input type for this function. <x> must be a list or a vector.", call. = FALSE)
  }
...
#' @importFrom magrittr %>%
#'
.strsplit_keep <- function(x, split, keep = 1, as_numeric = T) {
  
  # Separates strings and keeps first element of each vector
  x <- x %>%
    .strsplit(split = split, vector.output = F, as_numeric = as_numeric) %>%
...
#' @importFrom magrittr %>%
#'
.strsplit_keep_firstn <- function(x, split, n = 1, as_numeric = T) {
...
#' @importFrom magrittr %>%
#'
.strsplit_keep_last <- function(x, split, as_numeric = T) {
